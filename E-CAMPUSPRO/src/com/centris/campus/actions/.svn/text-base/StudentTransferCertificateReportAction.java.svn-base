...
public class StudentTransferCertificateReportAction extends DispatchAction{
	...
	public ActionForward getSection(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StudentTransferCertificateReportAction : getSection Starting");
		...
		try {
			...
			List<SectionPojo> seclist=new StudentTransferCertifivateReportBD().getSectionListDetails(classname);
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StudentTransferCertificateReportAction : getSection Ending");
