		  ...
		  List<ClassPojo> classlist =new StudentTransferCertifivateReportBD().getClassDetails();
			...
			if(onlinefeereceiptlist.size()>0){
				...
				try {
					...
					mapdata.put("StudentName", onlinefeereceiptlist.get(1).getStudentname());
					mapdata.put("classname", onlinefeereceiptlist.get(2).getClassname());
					mapdata.put("sectionName", onlinefeereceiptlist.get(3).getSectionname());
					mapdata.put("paiddate", onlinefeereceiptlist.get(4).getPaiddate());
					mapdata.put("totalamount", onlinefeereceiptlist.get(5).getTotalamount()+"");
					mapdata.put("termname", onlinefeereceiptlist.get(6).getTermname());
				
					String filepath = request
							.getRealPath("Reports/FeeReceiptPDFReport.jrxml");
