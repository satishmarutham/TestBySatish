...
public class StudentTransferCertifivateReportDaoImpl implements StudentTransferCertifivateReportDao {
	...
	public List<ClassPojo> getClassDetailsDao() {
		...
		try {
			...
			pstmt = conn
					.prepareStatement(ReportsMenuSqlConstants.GET_CLASS_DETAILS);
	...
	public List<SectionPojo> getSectionListDetailsDao(String classname) {
		
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StudentTransferCertifivateReportDaoImpl : getSectionListDetailsDao Starting");
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StudentTransferCertifivateReportDaoImpl : getSectionListDetailsDao Ending");
