...
public class DepartmentMasterDAOImpl implements DepartmentMasterDao {
	...
	public synchronized boolean validateDeptName(DepartmentMasterForm deptfForm) {
		...
		try {
			...
			department_pstmt = conn
					.prepareStatement(SQLUtilConstants.VALIDATE_DEPT_NAME);
	...
	public synchronized String deleteDepartmentDetails(String deptid) {
		...
		try {
			...
			deletedepartment_pstmt = conn
					.prepareStatement(SQLUtilConstants.CHECK_DEPARTMENT_MAP);

			deletedepartment_pstmt.setString(1, deptid);
			department_rs = deletedepartment_pstmt.executeQuery();

			while (department_rs.next()) {

				no = department_rs.getInt("deptname");
				...
				if (no > 0) {
					...
					ps_deletedepartment.setString(2, deptid);
					no = ps_deletedepartment.executeUpdate();

					if (no > 0) {

						status = MessageConstants.DEPARTMENT_DELETE_SUCCESS;
					} else {

						status = MessageConstants.DEPARTMENT_DELETE_FAIL;
					}
	...
	public synchronized ArrayList<DepartmentMasterVO> searchDepartment(
		...
		try {
			...
			department_pstmt
					.setString(1, "%" + searchvo.getSearch_name() + "%");
			department_pstmt
					.setString(2, "%" + searchvo.getSearch_name() + "%");
	...
	public synchronized boolean validateDeptNameUpdate(
		...
		try {
			...
			department_pstmt = conn
					.prepareStatement(SQLUtilConstants.VALIDATE_DEPARTMENT_UPDATE);
