	...
	
	{
			...
			
		{
			...
			try {
				...
				designation_pstmt.setString(1,"%"+vo.getDesgname()+"%");	
				
				designation_pstmt.setString(2,"%"+vo.getDesgname()+"%");				
...
public synchronized String insertDesignationDetails(AddDesignationPojo apojo) {
	...
	public synchronized String deleteDesignationDetails(AddDesignationVO vo)
	{
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		...
		try
		{
			...
			deletedesignation_pstmt.setString(1, vo.getDesgid());
			...
			if(no==0)
			{
				...
				ps_deletedesignation.setString(2, vo.getDesgid());
				...
				if(no>0)
				{
					...
					status=MessageConstants.DESIGNATION_SUCCESS;
