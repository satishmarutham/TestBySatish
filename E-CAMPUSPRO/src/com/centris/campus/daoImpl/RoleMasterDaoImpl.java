...
public class RoleMasterDaoImpl implements RoleMasterDao {
	...
	public synchronized String deleteRole(RoleMasterPojo roleMasterPojo)
		...
		try {
			...
			conn = JDBCConnection.getSeparateConnection();
			psdeleteRole = conn
					.prepareStatement(RoleMasterSqlConstants.CHECK_ROLE_USER_MAPPING);
			...
			if (result == 0) {
				psdeleteRole = conn
						.prepareStatement(RoleMasterSqlConstants.CHECK_ROLE_USER_ADMIN_CREATION);
				psdeleteRole.setString(1, roleMasterPojo.getRoleCode());
			...
			if (result > 0) {
				...
				if (result > 0) {
					...
					successMessage = "Role Not Deleted";
				}
