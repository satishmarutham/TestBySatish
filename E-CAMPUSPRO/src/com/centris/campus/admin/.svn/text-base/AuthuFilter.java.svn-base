...
public class AuthuFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		System.out
				.println("----------------------------------------------------------------------");
		if (wrappedRequest.getRequestURI().contains("/E-CAMPUSPRO/login.html")
				|| wrappedRequest.getRequestURI().contains(
						"/E-CAMPUSPRO/forgotPassword.html")) {
			System.out.println("Hi this is login page in filter");
			try {
				...
				"/E-CAMPUSPRO/displayNotice.html")) {
			System.out.println("Hi this is login page in filter");
			try {
				chain.doFilter(wrappedRequest, wrappedResponse);
			} catch (Throwable t) {
				...
				"/E-CAMPUSPRO/publicMenu.html")) {

			System.out.println("Hi this is welcome(Welcome) page  in filter");
			try {
				chain.doFilter(wrappedRequest, wrappedResponse);
			} catch (Throwable t) {
				problem = t;
			...
			try {
				...
				String ss = wrappedRequest.getSession().getAttribute("user")
						.toString();
				chain.doFilter(wrappedRequest, wrappedResponse);
				...
				pw.println("<html><head><script language='javascript'>"
						...
						+ "<form action='"
						+ request.getServletContext().getContextPath()
						+ "/publicMenu.html?method=Home' method='post' accept-charset='utf-8'><input type='submit' value='OK'/></form>"
						+ "</center></body></html>");
			}
		}
		if (problem != null) {
