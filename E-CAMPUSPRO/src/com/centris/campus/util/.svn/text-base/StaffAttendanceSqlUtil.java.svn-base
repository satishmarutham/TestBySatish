...
public class StaffAttendanceSqlUtil {
	...
	public static final String UPDATE_ATTENDANCE = "update campus_teacher_attendence set AttendenceStatus=?,UpdatedBy=?,UpdateDate=? where  TeacherID=? and AttendenceDate=?";
	public static final String INSERT_ATTENDANCE = "insert into campus_teacher_attendence(TeacherID,AttendenceDate,AttendenceStatus,isApproved,CreatedBy,CreateDate) values(?,?,?,?,?,?)";
	
	public static final String GET_ATTENDANCE_LIST = "select ta.AttendenceDate,count(t.TeacherID) as total_streangth,sum(case when ta.AttendenceStatus='Present' then 1 else 0 end) TotalPresent,sum(case when ta.AttendenceStatus='Absent' then 1 else 0 end) TotalAbsent,sum(case when ta.AttendenceStatus='Holiday' then 1 else 0 end) TotalHoliday,sum(case when ta.AttendenceStatus='Leave' then 1 else 0 end) Totalleave from campus_teacher_attendence ta,campus_teachers t where ta.AttendenceDate between ?  and ? and t.TeacherID=ta.TeacherID group by ta.AttendenceDate";
